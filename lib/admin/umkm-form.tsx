...
export function UmkmForm({
  ...
  return (
    <form action={action} className="mt-6 max-w-2xl space-y-5">
      ...
      <div>
        <label className="block text-sm font-medium">Link Foto (URL gambar)</label>
        <input name="foto_url" defaultValue={defaultValues.foto_url} className="mt-1 w-full rounded-lg border border-slate-300 px-3 py-2 text-sm" />
      </div>
